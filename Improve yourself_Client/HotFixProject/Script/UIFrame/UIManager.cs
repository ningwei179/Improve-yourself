...
public class UIManager : ILSingleton<UIManager>
{
    //UI节点
    ...
    /// <returns></returns>
    public void OpenUI<T>(string name, Action<bool, T> callback = null, AssetAddress resource = AssetAddress.Resources, params object[] paramList) where T : BaseUI
    {
        //开启遮罩避免开启UI的时候接收了操作出现异常
        SetMask(true);
        UnityEngine.Debug.Log("一个参数方法的调用1");
        BaseUI ui = FindUIByName<BaseUI>(name);
        if (ui == null)
        {
            UnityEngine.Debug.Log("一个参数方法的调用2");
            System.Type tp = null;
            if (m_RegisterDic.TryGetValue(name, out tp))
            {
                UnityEngine.Debug.Log("一个参数方法的调用3");
                //if (resource)
                //{
                ui = System.Activator.CreateInstance(tp) as BaseUI;
                ui.Init();
                UnityEngine.Debug.Log("一个参数方法的调用4");
                //}
                //else
                //{
            ...
            UnityEngine.Debug.Log("一个参数方法的调用5");
            GameObject uiObj = null;
            if (resource == AssetAddress.Resources)
            {
                UnityEngine.Debug.Log("一个参数方法的调用6");
                //从resource加载UI
            ...
            {
                UnityEngine.Debug.Log("一个参数方法的调用7");
                //从AssetBundle加载UI
            ...
            else if (resource == AssetAddress.Addressable)
            {
                ...
                //    InitPrefab(ui, uiObj, name, resource, paramList);
                //};
                UnityEngine.Debug.Log("一个参数方法的调用8");
                AddressableManager.AsyncInstantiate(m_UIPrefabPath + ui.PrefabName, (GameObject obj) =>
                {
                    UnityEngine.Debug.Log("一个参数方法的调用9");
                    uiObj = obj;
                    InitPrefab(ui, uiObj, name, resource, paramList);
                });
            }
            UnityEngine.Debug.Log("一个参数方法的调用10");
        }
        else
        {
            //设置UI的显示模式
    ...
    public void CloseUI(BaseUI ui, bool destory = false)
    {
        ...
        if (destory)
        {
            ...
            {
                AddressableManager.Release(ui.GameObject);
            }
